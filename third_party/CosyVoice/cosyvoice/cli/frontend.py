...
class CosyVoiceFrontEnd:
    ...
    def __init__(self,
        ...
        # The podcast has two fixed, licensed voice references. Their features can be
        # prepared once at build time, avoiding two large ONNX sessions in the 8 GiB pod.
        precomputed_path = os.environ.get('COSYVOICE_PRECOMPUTED_REFS', '')
        self.precomputed_refs = None
        self._validated_refs = set()
        if precomputed_path:
            self.precomputed_refs = torch.load(precomputed_path, map_location='cpu', weights_only=True)
            if not isinstance(self.precomputed_refs, dict) or not self.precomputed_refs:
                raise ValueError('empty CosyVoice reference feature cache')
    ...
    def _cached_ref(self, prompt_wav, key):
        if self.precomputed_refs is None:
            return None
        name = os.path.basename(os.fspath(prompt_wav))
        if name not in self.precomputed_refs:
            raise ValueError(f'no precomputed CosyVoice reference for {name}')
        if name not in self._validated_refs:
            with open(prompt_wav, 'rb') as stream:
                digest = hashlib.file_digest(stream, 'sha256').hexdigest()
            if digest != self.precomputed_refs[name]['sha256']:
                raise ValueError(f'CosyVoice reference changed without recomputing features: {name}')
            self._validated_refs.add(name)
        value = self.precomputed_refs[name][key]
        if isinstance(value, tuple):
            return tuple(t.to(self.device).clone() for t in value)
        return value.to(self.device).clone()

    def _extract_speech_token(self, prompt_wav):
        cached = self._cached_ref(prompt_wav, 'speech_token')
    ...
    def _extract_spk_embedding(self, prompt_wav):
        cached = self._cached_ref(prompt_wav, 'spk_embedding')
        if cached is not None:
            return cached
    ...
    def _extract_speech_feat(self, prompt_wav):
        cached = self._cached_ref(prompt_wav, 'speech_feat')
        if cached is not None:
            return cached
